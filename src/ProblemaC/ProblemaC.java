...
public class ProblemaC {
	...
	/**
	 * Calculates the successors of the given state. Successors are all states agregando un servicio que no este en el subconjunto y que no se cruce
	...
	private List<subconjunto> sucesores(subconjunto state) {
		...
		int[] successors = new int[s.length];
		
		for(int i=0;i<s.length;i++) {
			subconjunto suc = new subconjunto(s);
			
			if(!cruce() && s.contains(s))
				successors.add(suc);
		}
		return successors;
	}
